...
Dockerized PHP project setup tool with automatic port assignment, SSL certificates, and local domain configuration. Supports Laravel Sail, pre-Sail Laravel (6/7/8), and CakePHP 2 via interchangeable presets.

## Features

- 🧰 **Presets** - Pick `sail`, `laravel-legacy`, or `cakephp-2` at init time; the right stack gets scaffolded
- 🚢 **Automatic Port Assignment** - Intelligent port allocation across multiple projects
...
- 📦 **Composer Installation** - Docker-based Composer image, version auto-detected from `composer.lock`; supports private repository authentication via `auth.json`
...
- **Project**: A Laravel Sail project (existing `docker-compose.yml`), a pre-Sail Laravel project, or a CakePHP 2 project
...
1. **Preset Selection** - Pick `sail`, `laravel-legacy`, or `cakephp-2` (see [Presets](#presets) below)
2. **Stack Choices** (legacy presets) - PHP version, MySQL version, and optional services (Meilisearch / Elasticsearch with image tag)
3. **Cake-2 Docroot** (cakephp-2 only) - Confirm the detected web docroot (`public_html`, `app/webroot`, or `webroot`)
4. **Composer Authentication** - Provide credentials for private repositories (stored next to `composer.json` as `auth.json`)
5. **Port Assignment** - Automatic detection and conflict resolution
6. **Stack Scaffolding** (legacy presets) - Writes `docker-compose.yml`, `Dockerfile`, `nginx.conf`, `php.ini`, and for cake-2 clones/patches the env folder and generates `local.php`
7. **Domain Registration** (Optional) - Register a `.test` domain via Valet/Herd
8. **Protocol Selection** (If domain registered) - Choose between HTTPS (secure) or HTTP
9. **SSL Certificates** (If HTTPS selected) - Automatic certificate setup
10. **Post-Setup Commands** (Optional) - Start containers and run Composer setup
11. **Review Checklist** - Every file created or patched is listed at the end for manual review

## Presets

The first wizard question picks one of three presets. Each maps to a different Docker stack and a different set of project-shape assumptions.

### `sail` — existing Laravel Sail project (unchanged from 0.4.x)

Use when the repo already has a `docker-compose.yml` generated by Sail. Shipyard only assigns ports, configures `.env`, registers a domain, and runs composer install. **Required:** existing `docker-compose.yml`, `composer.json`, and either `.env.example` or no `.env`.

### `laravel-legacy` — pre-Sail Laravel 6/7/8 on PHP 7.2/7.3/7.4

Use when the repo has a Laravel 6/7/8 codebase but no Docker setup. Shipyard scaffolds:

- `docker-compose.yml` with php (php-fpm), nginx, mysql, redis, mailpit (+ optional meilisearch / elasticsearch)
- `Dockerfile` based on `php:${PHP_VERSION}-fpm` with the Laravel-required extensions installed via `mlocati/docker-php-extension-installer`
- `nginx.conf` with docroot `public/`
- `php.ini` with dev-friendly settings (`display_errors=On`, etc.)
- An empty `.env` if neither `.env` nor `.env.example` exists, populated with `COMPOSE_PROJECT_NAME`, `APP_URL`, and assigned ports

**Required:** existing `composer.json`.

### `cakephp-2` — legacy CakePHP 2 on PHP 5.6/7.x

Use for legacy CakePHP 2 projects. Shipyard scaffolds a slimmer Docker stack than `laravel-legacy` — php-fpm + nginx + mysql + mailpit, plus the same optional Meilisearch / Elasticsearch services (no redis, different PHP extension set). It also:

- Detects the project layout: `app/Config/` or top-level `Config/`
- Detects the web docroot: probes `public_html/` → `app/webroot/` → `webroot/`, asks for confirmation
- Detects a source env folder under the config dir (prefers `dev/`, otherwise the first folder with `bootstrap.php` + `database.php`, skipping `docker/`)
- Clones the source env folder to `app/Config/docker/` (prompts before overwriting)
- Auto-patches the cloned files:
  - `database.php` `$default` array — host → `mysql`, login → `sail`, password → `password`, database → `<project>` (other configs like `$test` left untouched)
  - `email.php` — every `host` → `mailpit`, every `port` → `1025`
  - `bootstrap.php` + `config.php` — `define('SITE_URL', …)` and the first `'url' => 'http(s)://…'` rewritten to `http://<project>.test/`
- Generates `app/Config/local.php` selecting the new docker env (prompts before overwriting)
- Composer install accommodates `composer.json` at either the repo root or `app/composer.json`, and is skipped entirely if neither exists

The "Review checklist" at the end of init lists every cloned/patched file so you can verify the best-effort rewrites before committing.

## Template versioning

Stack templates (`docker-compose.yml`, `Dockerfile`, etc.) are fetched from GitHub at scaffold time, pinned to the release tag matching the installed Shipyard version. To test or override (e.g. while running an unreleased build):

```bash
SHIPYARD_TEMPLATE_REF=feature/my-branch shipyard init
```

`SHIPYARD_TEMPLATE_REF` accepts any branch name, tag, or commit SHA on `wotzebra/shipyard`.
...
├── TESTING.md         # Recipes for exercising each preset locally
├── LICENSE            # MIT license
├── shipyard.sh        # Main script
├── install.sh         # Installation script
└── templates/         # Stack templates fetched at scaffold time (legacy presets)
```
...
For end-to-end testing of each preset (Sail, Laravel Legacy, CakePHP 2) against fixtures or real projects, see [TESTING.md](TESTING.md).
